...
function readDotEnvFile(envPath) {
  ...
  try {
    ...
    // Ignore malformed env and proceed with settings.json values.
  }
  return parsed;
}

function withTwitterEnvOverrides(config) {
  const envFromFile = readDotEnvFile(path.join(getBotDir(), '.env'));
  if (!envFromFile || Object.keys(envFromFile).length === 0) return config;
...
function writeEnvFile(config) {
  ...
  const lines = [
    ...
    `SOLANA_RPC_CONSTANTK=${config.SOLANA_RPC_CONSTANTK || ''}`,
    `SOLANA_RPC_HTTP=${config.SOLANA_RPC_HTTP || ''}`,
    `SOLANA_WSS_URL=${config.SOLANA_WSS_URL || ''}`,
    `SOLANA_RPC_QUICKNODE=${config.SOLANA_RPC_QUICKNODE || ''}`,
    `SOLANA_RPC_PUBLIC=https://api.mainnet-beta.solana.com`,
    `SOLANA_ALLOW_PUBLIC_RPC_FALLBACK=${config.SOLANA_ALLOW_PUBLIC_RPC_FALLBACK === true ? 'true' : 'false'}`,
    ...
    `TWITTER_MIN_FOLLOWERS=${config.TWITTER_MIN_FOLLOWERS || '8000'}`,
    `TWITTER_MAX_DISCOVERED_HANDLES=${config.TWITTER_MAX_DISCOVERED_HANDLES || '20'}`,
...
function startBot() {
  ...
  const config = withTwitterEnvOverrides(loadConfig());
  const botDir = getBotDir();
  writeEnvFile(config);

  const botScript = path.join(botDir, 'dist', 'bot.cjs');
...
function buildBotEnv(config) {
  ...
  return {
    ...
    SOLANA_WALLET_PUBLIC_KEY: parsed.WALLET_PUBLIC_KEY || '11111111111111111111111111111111',
    WALLET_PRIVATE_KEY: parsed.WALLET_PRIVATE_KEY || '',
    SOLANA_RPC_CONSTANTK: parsed.SOLANA_RPC_CONSTANTK || '',
    SOLANA_RPC_HTTP: parsed.SOLANA_RPC_HTTP || '',
    SOLANA_WSS_URL: parsed.SOLANA_WSS_URL || '',
    SOLANA_RPC_QUICKNODE: parsed.SOLANA_RPC_QUICKNODE || '',
    SOLANA_RPC_PUBLIC: 'https://api.mainnet-beta.solana.com',
    SOLANA_ALLOW_PUBLIC_RPC_FALLBACK: parsed.SOLANA_ALLOW_PUBLIC_RPC_FALLBACK === 'true' ? 'true' : 'false',
    ...
    TWITTER_AUTO_KOL_DISCOVERY: parsed.TWITTER_AUTO_KOL_DISCOVERY || 'true',
    TWITTER_DISCOVERY_KEYWORDS: parsed.TWITTER_DISCOVERY_KEYWORDS || 'pump.fun solana,solana ca,solana gem,raydium migration,new solana pair',
    TWITTER_MIN_FOLLOWERS: parsed.TWITTER_MIN_FOLLOWERS || '8000',
    TWITTER_MAX_DISCOVERED_HANDLES: parsed.TWITTER_MAX_DISCOVERED_HANDLES || '20',
    TWITTER_COOKIES: parsed.TWITTER_COOKIES || '',
...
// ── IPC Handlers ──
ipcMain.handle('config:load', () => withTwitterEnvOverrides(loadConfig()));
